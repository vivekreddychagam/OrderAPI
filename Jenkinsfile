node {
   ...
   {
       sh 'curl -v -X POST --data-binary @deploy/pumrporderdeploy.yaml -H "Content-Type: application/x-yaml" 54.91.104.221:8080/api/v1/deployments'
   }
   stage('Move GW 50/50') 
   {
       input 'Do you approve deployment?'
       sh 'curl -v -X PUT --data-binary @deploy/updategw50.yaml -H "Content-Type: application/x-yaml" 54.91.104.221:8080/api/v1/gateways/pumrpapi'
   }
   stage('Move Full') 
   {
       input 'Do you approve deployment?'
       sh 'curl -v -X PUT --data-binary @deploy/updategw100.yaml -H "Content-Type: application/x-yaml" 54.91.104.221:8080/api/v1/gateways/pumrpapi'
   }
   stage('Undeploy Previous App') 
   {
       sh 'curl -v -X DELETE --data-binary @deploy/pumrporderpredeploy.yaml -H "Content-Type: application/x-yaml" 54.91.104.221:8080/api/v1/deployments/pumrporder:$((${BUILD_ID}-1))'
